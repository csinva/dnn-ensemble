...
def accuracy_topk(self, logits, targets, topk=1):
    """Returns topk accuracy of given logits and targets"""
...
def find_correct(self, logits, targets, topk=1):
...
def get_pred(self, logits, topk=1):
    """Returns topk predictions for passed in logits"""
    _, pred = logits.topk(topk, 1, True, True)
    return pred.t()

class DNNUtil:
    
    def __init__(self, models_to_load, eval_settings, logdir):
        self.eval_settings = eval_settings
        self.logdir = logdir
        
        # check to see if any logits are missing 
        ...
        for model in tqdm(models_to_load, desc='load_logits_targets', leave=False):
            for eval_setting in ['val', 'imagenetv2-matched-frequency']:
                ...
                if output_folder in output_folders:
                    ...
                    model_logits = os.path.join(path_to_imagenet, output_folder, 'logits.pt')
                    if os.path.exists(model_logits):
                        self.logits[eval_setting][model] = torch.load(model_logits)
                    if eval_setting not in targets and os.path.exists(model_targets):
                        self.targets[eval_setting] = torch.load(model_targets)
    ...
    def partition(self, data, eval_setting='val'):
        return train_test_split(data, test_size=0.5, stratify=self.targets[eval_setting], random_state=42)

    
    def view_image(self, index, eval_setting='val'):
        ...
        num_img_per_class = self.targets[eval_setting].size(0) // 1000
    ...
    def get_ensemble_logits(self, softmax_pred):
    ...
    def ensemble_models(self, models, ensemble_name, eval_setting='val'):
        softmax_pred, pred = [], []

        for model in models:
            softmax_pred.append(softmax(self.logits[eval_setting][model], dim=1))
        softmax_pred = torch.stack(softmax_pred)

        self.logits[eval_setting][ensemble_name] = self.get_ensemble_logits(softmax_pred)

        
    def majority_vote_models(self, models, ensemble_name, eval_setting='val'):
        pred = get_pred(logits[eval_setting][models[0]])
        for model in models[1:]:
            pred = torch.cat((pred, get_pred(self.logits[eval_setting][model])), 0)
        self.logits[eval_setting][ensemble_name] = torch.mode(pred, 0, keepdim=True)[0] 
    
    
    def class_weighted_ensemble(self, models, ensemble_name, eval_setting='val'):
    """Weights each model's softmax predictions based on its relative accuracy in the class.
    Uses half of each class' images to calculate in class accuracy."""
    w = [] 
    for model in models:
        class_acc = []
        pred = get_pred(self.logits[eval_setting][model])
        miss = pred.eq(self.targets[eval_setting].view(1, -1).expand_as(pred)).float().flatten()
        total_images = self.targets[eval_setting].size(0)
    ...
    for i, model in enumerate(models):
        probs = softmax(self.logits[eval_setting][model], dim=1)
    ...
    self.logits[evla_setting][ensemble_name] = get_ensemble_logits(weighted_softmax_pred)
    
    def acc_weighted_ensemble(self, models, ensemble_name, eval_setting='val'):
    ...
    eval_targets_train, eval_targets_test = self.partition(self.targets[eval_setting], eval_setting)

    weighted_softmax_pred = []
    for model in models:
        train_logit, _ = self.partition(self.logits[eval_setting][model], eval_setting)
    ...
    weighted_softmax_pred = torch.stack(weighted_softmax_pred)
    return self.logits[eval_setting][ensemble_name] = self.get_ensemble_logits(weighted_softmax_pred)
